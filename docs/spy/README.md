...
The script reads the upstream version from `apps/server/package.json` and chooses the next
`-spy.N` suffix from GitHub releases (including drafts) and remote tags. For example, after
`0.0.40-spy.3` it chooses `0.0.40-spy.4`; after updating upstream to `0.0.41`, it starts at
`0.0.41-spy.1`. It prints the selected version before building. To see the next version without
...
`.t3\build-desktop-win.cmd --publish 0.0.40-spy.7`.
