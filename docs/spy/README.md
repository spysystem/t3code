...
- **Desktop releases are unsigned local Windows and Linux x64 builds**, published to public GitHub Releases
  ...
  exact tested files requires a separate maintainer request. Packaged Windows x64 builds check for SPY releases and link to
...
including an interrupted build, reserves its version on that machine.
