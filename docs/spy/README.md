...
- **Desktop releases are unsigned local Windows and Linux x64 builds**, published to public GitHub Releases
  by the local build script. Packaged Windows x64 builds check for SPY releases and link to
...
To build and publish Windows x64 and Linux x64 together in one step, run:
...
`-NextVersion`.
The publisher builds Linux in WSL, then calls `.t3\build-desktop-win.cmd` without arguments
...
The artifact is unsigned, so SmartScreen warns on first run. Packaged Windows x64 builds with a
`-spy.N` version check the fork's latest published release after startup and every four hours.
They offer a GitHub release link for manual download, independently of the electron-updater feed.
No automatic download or installation is enabled; `T3CODE_DISABLE_AUTO_UPDATE` also disables
these checks. Existing builds that report "no update feed is configured" need one manual upgrade
...
refreshes a separate checkout at `~/build/t3code-linux` from the exact Windows source commit,
...
The publisher copies only the four public Connect settings from the Windows `.env`; it never
copies the reusable dev credential. Keep `.env.local` out of the managed build checkout.

For another configured distribution or checkout location, call the publisher directly:
...
WSL/kernel, distro, and Rust/Vite+ upgrades are explicit maintenance operations; publishing does
not run system upgrades or restart WSL. A failed build on either platform stops publication.

## Desktop distribution

The local publisher uploads the `.exe`, `.AppImage`, and a `SHA256SUMS.txt` covering both to a draft release
