...
Windows and Linux check for a matching `.exe` or `.AppImage` asset; Macs only need the release,
because they rebuild it. They offer a GitHub release link for manual
updates, independently of the electron-updater feed.
...
those distributions. Apple Silicon Macs build releases locally (see above); a published, signed
