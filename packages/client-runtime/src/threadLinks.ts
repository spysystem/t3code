...
export function resolveThreadLinkRules(
  settings:
    | (Pick<ServerSettings, "defaultThreadLinkRules" | "projectThreadLinkOverrides"> &
        Partial<Pick<ServerSettings, "projectSettingsOverrides">>)
    | null,
  projectId: ProjectId,
): ReadonlyArray<ThreadLinkRule> {
  return (
    settings?.projectSettingsOverrides?.[projectId]?.defaultThreadLinkRules ??
