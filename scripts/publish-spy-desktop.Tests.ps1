...
Describe 'Desktop test candidates and publication' {
    function git {
    ...
    It 'builds both artifacts and matching checksums without publishing' {
    ...
    It 'rejects invalid versions before doing any work' {
        { & $global:spyPublishpublisher -Version 'invalid-version' -ErrorAction Stop } | Should Throw 'Cannot validate argument'
        $global:spyPublishevents.Count | Should Be 0
    }

    It 'publishes the saved candidate without building or depending on current source and build settings' {
        & $global:spyPublishpublisher -Version '0.0.40-spy.1'
        $global:spyPublishevents = @()
        $global:spyPublishscenario = 'dirty'
        Remove-Item -LiteralPath "$fixture/.env", "$fixture/.t3/build-desktop-win.cmd"
        & $global:spyPublishpublisher -CandidateDirectory $global:spyPublishoutputDirectory
        ($global:spyPublishevents -join ',') | Should Be 'create,edit'
        $global:spyPublishCreatedTag | Should Be 'spy-v0.0.40-spy.1'
    }

    It 'builds unpushed source but requires it on spy/main before publication' {
        $global:spyPublishscenario = 'unpushed'
        & $global:spyPublishpublisher -Version '0.0.40-spy.1'
        { & $global:spyPublishpublisher -CandidateDirectory $global:spyPublishoutputDirectory } | Should Throw 'Push the candidate source commit'
        ($global:spyPublishevents -join ',') | Should Be 'linux-build,build'
    }

    foreach ($file in @('T3-Code-0.0.40-spy.1-x64.exe', 'T3-Code-0.0.40-spy.1-x86_64.AppImage', 'SHA256SUMS.txt', 'release-notes.md')) {
        It "rejects modified $file before publishing" {
            & $global:spyPublishpublisher -Version '0.0.40-spy.1'
            Set-Content -LiteralPath (Join-Path $global:spyPublishoutputDirectory $file) -Value 'changed bytes'
            { & $global:spyPublishpublisher -CandidateDirectory $global:spyPublishoutputDirectory } | Should Throw 'changed after building'
            ($global:spyPublishevents -join ',') | Should Be 'linux-build,build'
        }
    }

    It 'rejects an incomplete candidate before publishing' {
        & $global:spyPublishpublisher -Version '0.0.40-spy.1'
        Remove-Item -LiteralPath $global:spyPublishAppImage
        { & $global:spyPublishpublisher -CandidateDirectory $global:spyPublishoutputDirectory } | Should Throw 'Missing candidate file'
        ($global:spyPublishevents -join ',') | Should Be 'linux-build,build'
    }

    It 'refuses to replace an existing release after testing' {
        & $global:spyPublishpublisher -Version '0.0.40-spy.1'
        $global:spyPublishscenario = 'existing'
        { & $global:spyPublishpublisher -CandidateDirectory $global:spyPublishoutputDirectory } | Should Throw 'already exists'
        ($global:spyPublishevents -join ',') | Should Be 'linux-build,build'
    }

    It 'reserves versions locally so a second test build has a new version' {
        & $global:spyPublishpublisher
        & $global:spyPublishpublisher -NextVersion | Should Be '0.0.41-spy.2'
        { & $global:spyPublishpublisher -Version '0.0.41-spy.1' } | Should Throw 'already exists'
        & $global:spyPublishpublisher
        [IO.Path]::GetFileName($global:spyPublishinstaller) | Should Be 'T3-Code-0.0.41-spy.2-x64.exe'
        ($global:spyPublishevents -contains 'create') | Should Be $false
    }

    It 'builds even when the optional Get-FileHash cmdlet cannot load' {
        function Get-FileHash { throw 'Get-FileHash is unavailable in the build shell.' }
        & $global:spyPublishpublisher -Version '0.0.40-spy.1'
        ($global:spyPublishevents -join ',') | Should Be 'linux-build,build'
    }

    It 'builds the first version for the current upstream base without publishing' {
        & $global:spyPublishpublisher
        [IO.Path]::GetFileName($global:spyPublishinstaller) | Should Be 'T3-Code-0.0.41-spy.1-x64.exe'
        ($global:spyPublishevents -join ',') | Should Be 'linux-build,build'
    }
    ...
    It 'preserves an explicit version override' {
        ...
        [IO.Path]::GetFileName($global:spyPublishinstaller) | Should Be 'T3-Code-0.0.40-spy.7-x64.exe'
    }
    ...
    foreach ($failure in @('dirty', 'existing', 'unauthenticated')) {
    ...
    It 'keeps the release unpublished if upload fails' {
        ...
        { & $global:spyPublishpublisher -CandidateDirectory $global:spyPublishoutputDirectory } | Should Throw 'gh failed with exit code 1'
    ...
    foreach ($failure in @('wsl-unavailable', 'linux-build-failed', 'missing-appimage')) {
        It "does not build Windows or publish when $failure" {
            $global:spyPublishscenario = $failure
            $reason = if ($failure -eq 'missing-appimage') { 'expected AppImage' } else { 'wsl.exe failed' }
            { & $global:spyPublishpublisher -Version '0.0.40-spy.1' } | Should Throw $reason
    ...
    It 'restores ambient Connect settings after building' {
    ...
    It 'rejects a concurrent release from the same Windows checkout' {
        ...
        try {
            { & $global:spyPublishpublisher -Version '0.0.40-spy.1' } | Should Throw 'being used by another process'
            $global:spyPublishevents.Count | Should Be 0
        }
