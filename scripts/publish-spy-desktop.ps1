...
function Get-NextVersion {
    param([string[]]$Tags)
    $baseVersion = (Get-Content -LiteralPath 'apps/server/package.json' -Raw | ConvertFrom-Json).version
    if ($baseVersion -notmatch '^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$') {
        throw "Cannot choose a release version from upstream version '$baseVersion'. Supply -Version explicitly."
    }
