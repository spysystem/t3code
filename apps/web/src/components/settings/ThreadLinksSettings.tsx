...
export function ThreadLinksSettings(scope: Scope) {
  ...
  const projectOverrideCount = scope.projects
    ? 0
    : connected.reduce(
        (count, target) =>
          count +
          Object.keys(target.serverConfig?.settings.projectThreadLinkOverrides ?? {}).length,
        0,
      );
    ...
    for (const target of connected) {
      const patch = scope.projects
        ...
        : {
            ...
            // All projects includes projects with explicit rules, including disabled links.
            // Clearing those entries makes existing and future projects follow the new default.
            ...(Object.keys(target.serverConfig?.settings.projectThreadLinkOverrides ?? {}).length >
            0
              ? {
                  projectThreadLinkOverrides: Object.fromEntries(
                    Object.keys(target.serverConfig?.settings.projectThreadLinkOverrides ?? {}).map(
  ...
  return (
    <SettingsSection title="Thread links" id="thread-links">
      <SettingsRow
        title={scope.projects ? "Project thread links" : "Thread links for all projects"}
        description={
          unavailable
            ? "Connect the selected environments to edit thread links."
            : !scope.projects
              ? `Saving replaces rules for all projects on the selected connected environments${projectOverrideCount > 0 ? `, including ${projectOverrideCount} project-specific overrides` : ""}. Offline environments keep their current rules.`
              : mixed
                ? "Rules differ across the selected projects or environments. Saving replaces the rules in the selected scope."
                : inherited
