...
describe("SPY releases", () => {
  ...
  it.effect("requires a non-empty installer matching the current platform", () =>
    Effect.gen(function* () {
      for (const platform of ["win32", "linux"] as const) {
        const suffix = platform === "linux" ? "x86_64.AppImage" : "x64.exe";
        const otherSuffix = platform === "linux" ? "x64.exe" : "x86_64.AppImage";
        for (const asset of [
          { name: `T3-Code-0.0.41-spy.10-${otherSuffix}`, size: 100 },
          { name: `T3-Code-0.0.41-spy.10-${suffix}`, size: 0 },
          { name: `T3-Code-0.0.41-spy.9-${suffix}`, size: 100 },
          { name: "T3-Code-0.0.41-spy.10-arm64.AppImage", size: 100 },
        ]) {
          fetchMock.mockResolvedValue(Response.json({ ...release(), assets: [asset] }));
          const error = yield* checkSpyRelease("0.0.41-spy.9", platform).pipe(Effect.flip);
  ...
  it.effect("offers Macs any complete release because they rebuild it from source", () =>
    Effect.gen(function* () {
      fetchMock.mockResolvedValue(Response.json({ ...release(), assets: [] }));
      const update = yield* checkSpyRelease("0.0.41-spy.9", "darwin");
      assert.deepEqual(update, { version: "0.0.41-spy.10", url: release().html_url });
      fetchMock.mockResolvedValue(Response.json({ ...release(), draft: true }));
      assert.equal(
        (yield* checkSpyRelease("0.0.41-spy.9", "darwin").pipe(Effect.flip))._tag,
        "SpyReleaseCheckError",
      );
      fetchMock.mockResolvedValue(Response.json(release()));
      assert.equal(
        (yield* checkSpyRelease("0.0.41-spy.9", "freebsd").pipe(Effect.flip))._tag,
        "SpyReleaseCheckError",
      );
    }).pipe(Effect.provideService(FetchHttpClient.Fetch, fetchMock)),
  );
