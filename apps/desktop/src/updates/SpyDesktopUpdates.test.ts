  ...
  it.effect("does not check in development, on unsupported platforms, or when disabled", () =>
    Effect.gen(function* () {
      ...
      for (const options of [
        ...
        { ...spyOptions, platform: "darwin" as const, processArch: "x64" },
        { ...spyOptions, processArch: "arm64" },
...
describe("SPY desktop manual updates on Apple Silicon Macs", () => {
  it.effect("offers the release to rebuild without a published Mac installer", () => {
    const fetchMock = stubRelease("win32");
    const harness = makeHarness({ ...baseSpyOptions, platform: "darwin", processArch: "arm64" });
    return Effect.scoped(
      Effect.gen(function* () {
        const updates = yield* DesktopUpdates.DesktopUpdates;
        yield* updates.configure;
